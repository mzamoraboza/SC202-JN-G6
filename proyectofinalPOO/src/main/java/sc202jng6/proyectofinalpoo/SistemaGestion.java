...
public class SistemaGestion {
    ...
    public void menuAdministrador() {
        ...
        switch (opcion) {
            case 1 -> {
                ...
                int id = usuarioCount + 1; // Asignar un ID único al usuario
                Usuario nuevoUsuario = new Usuario(id, nombre, correo, password, RolUsuario.Administrador);
                agregarUsuario(nuevoUsuario);
                JOptionPane.showMessageDialog(null, "Usuario agregado exitosamente: " + nuevoUsuario.getNombre(), "Usuario Agregado", JOptionPane.INFORMATION_MESSAGE);
            ...
            case 2 -> {
                ...
                nuevoCurso.setIdCurso(cursoCount + 1); // Asignar un ID único al curso
                agregarCurso(nuevoCurso);
                JOptionPane.showMessageDialog(null, "Curso agregado exitosamente: " + nuevoCurso.getnombreCurso(), "Curso Agregado", JOptionPane.INFORMATION_MESSAGE);
            ...
            case 3 -> {
                ...
                int idMatricula = matriculaCount + 1;
    ...
    public void agregarUsuario(Usuario usuario) {
        if (usuarioCount < usuarios.length) {
            ...
            JOptionPane.showMessageDialog(null, "No se pueden agregar más usuarios.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    // Método para agregar una matrícula al sistema
    public void agregarMatricula(Matricula matricula) {
        if (matriculaCount < matriculas.length) {
            matriculas[matriculaCount++] = matricula;
        } else {
            JOptionPane.showMessageDialog(null, "No se pueden agregar más matrículas.", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }

    // Método para agregar un curso al sistema
    public void agregarCurso(Curso curso) {
        if (cursoCount < cursos.length) {
